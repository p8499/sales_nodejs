<template>
    <div class="pac-table" style="max-width: 100%; max-height: 100%;">
        <div>
            <div :style="{display: paging? 'flex' : 'none'}">
                <span v-if="contentRange.total > 0">记录 {{contentRange.start + 1}} - {{contentRange.end + 1}} / {{contentRange.total}}</span>
                <span v-else>无记录</span>
                <img style="margin-left: 4px;" src="" alt="" @click="$emit('refresh')"/>
            </div>
            <div :style="{display: paging? 'flex' : 'none'}"
                 :class="{'has-previous': contentRange.pageNumber(pageSize) > 1, 'has-next': contentRange.pageNumber(pageSize) < contentRange.pageMax(pageSize)}">
                <img src="" alt="" @click="goto(1)"/>
                <img src="" alt="" @click="goto(contentRange.pageNumber(pageSize)-1)"/>
                <input :value="contentRange.pageNumber(pageSize)" @input="goto(parseInt($event.target.value))"/>
                <img src="" alt="" @click="goto(contentRange.pageNumber(pageSize)+1)"/>
                <img src="" alt="" @click="goto(contentRange.pageMax(pageSize))"/>
                ...
                <span style="margin-left: 4px;">行 / 每页</span>
            </div>
        </div>
        <div ref="head-div">
            <table>
    ...
    export default class PacTable<T> extends Vue {
        ...
        drag(event: MouseEvent) {
            ...
            if (this.dragCol !== undefined && this.dragStartClientX !== undefined && this.dragStartWidth !== undefined) {
                (headColgroup.children[this.dragCol] as HTMLTableColElement).width = `${this.dragStartWidth + event.clientX - this.dragStartClientX}px`;
                (bodyColgroup.children[this.dragCol] as HTMLTableColElement).width = `${this.dragStartWidth + event.clientX - this.dragStartClientX}px`;
                this.resize();
            }
        }
