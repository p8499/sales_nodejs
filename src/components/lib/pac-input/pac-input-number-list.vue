    ...
    <div class="pac-input-list">
        ...
        <div :style="{'margin-left': label !== null? '120px' : ''}">
            <div v-for="(v, i) in value" :key="i">
                <input class="right" :value="v.toFixed(fraction)" readonly tabindex="-1"/>
                <img style="margin-left: 1px;" src="" alt=""
...
<script lang="ts">
    ...
    export default class PacInputNumberList extends PacInputCommon {
        ...
        @Watch('rawValue')
        async onRawValueChange(newVal: string): Promise<void> {
            //检测数字格式不正确
            const sections = newVal.split(this.separator).filter(v => v !== '');
